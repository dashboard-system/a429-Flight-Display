  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setFlightData((prev) => {
        ...
        const densityAltitude = prev.altitude + (prev.static_air_temperature - 15) * 120;
        ...
        // Step climb scenario (common in cruise)
        const stepClimbTrigger = Math.random() < 0.002; // 0.2% chance per update
        const stepClimbRate = stepClimbTrigger && phase === 'CRUISE' ? 300 : 0;
        ...
        // Autopilot stability in cruise
        const autopilotActive = phase === 'CRUISE' || phase === 'CLIMB' || phase === 'DESCENT';
        const stabilityFactor = autopilotActive ? phaseParams.stabilityFactor : 0.5;
